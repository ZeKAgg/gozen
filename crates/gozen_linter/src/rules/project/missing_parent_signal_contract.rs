...
use super::parent_contract_support::{
    extract_parent_signal_names, parent_has_signal, resolve_parent_candidates_for_attachment,
};
use crate::rule::{ProjectRule, RuleMetadata};

pub struct MissingParentSignalContract;

const METADATA: RuleMetadata = RuleMetadata {
    id: "correctness/missingParentSignalContract",
    name: "missingParentSignalContract",
    group: "correctness",
    default_severity: Severity::Warning,
    has_fix: false,
    description: "Child script expects parent signal contract that host parents do not provide.",
    explanation: "When a child scene script connects to get_parent().connect(\"signal\", ...), each resolved host parent script must define that signal. Gozen reports only provable violations and skips ambiguous placements.",
};

impl ProjectRule for MissingParentSignalContract {
    ...
    fn check(
        ...
        let contracts = extract_parent_signal_names(tree, source);
        for contract in contracts {
            for attachment in &attachments {
                ...
                for candidate in &candidates {
                    ...
                    let Some(has_signal) = parent_has_signal(graph, candidate, &contract.value)
                    else {
                        ambiguous = true;
                        break;
                    };
                ...
                diags.push(Diagnostic {
                    ...
                    message: format!(
                        "Parent signal contract missing: get_parent().connect(\"{}\", ...) expects parent signal in host scene(s): {}.",
                        contract.value,
                        host_list.join(", ")
                    ),
                    file_path: None,
...
mod tests {
    ...
    fn with_host(
        ...
        graph.scripts.insert(
            ...
            ScriptData {
                ...
                class_name: None,
                signals: parent_signals.into_iter().map(|s| s.to_string()).collect(),
                functions: vec!["_ready".to_string()],
                exported_vars: Vec::new(),
    ...
    #[test]
    fn missing_parent_signal_contract_reports_when_parent_signal_absent() {
        let source = r#"func _ready():
    get_parent().connect("missing_signal", Callable(self, "_on_any"))
"#;
        let (graph, tree) = base_graph(source);
        let graph = with_host(graph, "res://scripts/parent.gd", vec![]);
        let diags =
            MissingParentSignalContract.check(&tree, source, &graph, "res://scripts/child.gd");
        assert_eq!(diags.len(), 1);
        assert!(diags[0].message.contains("missing_signal"));
    }

    #[test]
    fn missing_parent_signal_contract_skips_when_parent_signal_exists() {
        let source = r#"func _ready():
    get_parent().connect("ready_signal", Callable(self, "_on_any"))
"#;
        let (graph, tree) = base_graph(source);
        let graph = with_host(graph, "res://scripts/parent.gd", vec!["ready_signal"]);
        let diags =
            MissingParentSignalContract.check(&tree, source, &graph, "res://scripts/child.gd");
    ...
    #[test]
    fn missing_parent_signal_contract_skips_non_literal_signal_names() {
        let source = r#"func _ready():
    get_parent().connect(signal_name, Callable(self, "_on_any"))
"#;
        let (graph, tree) = base_graph(source);
        let graph = with_host(graph, "res://scripts/parent.gd", vec![]);
        let diags =
    ...
    #[test]
    fn missing_parent_signal_contract_reports_against_builtin_parent() {
        let source = r#"func _ready():
    get_parent().connect("missing_signal", Callable(self, "_on_any"))
"#;
    ...
    #[test]
    fn missing_parent_signal_contract_deduplicates_same_contract_for_multiple_attachments() {
        let source = r#"func _ready():
    get_parent().connect("missing_signal", Callable(self, "_on_any"))
"#;
        let mut parser = GDScriptParser::new();
